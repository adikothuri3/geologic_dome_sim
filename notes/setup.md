...
- **`setuptools<81` is a load-bearing pin in both installers** (found 2026-08-09 on the first Colab run). `flatdict==4.0.1`, an `isaaclab` dependency, has no wheel on PyPI, so pip builds it from sdist and its `setup.py` does `import pkg_resources` — which setuptools 81 dropped. `--no-build-isolation` alone is **not** the fix: it only moves the build into the venv, so the venv's own setuptools must still carry `pkg_resources`. Measured in a clean 3.11 venv: setuptools **84.0.0** → `ModuleNotFoundError: No module named 'pkg_resources'`, `metadata-generation-failed`; setuptools **80.10.2** → installs clean. This box never hit it because Norton was silently failing the `--upgrade setuptools` step, leaving python's bundled **65.5.0** in place — a masked bug, not an absent one.
