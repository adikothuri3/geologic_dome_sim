...
3. **Terrain build** — Open3D cleanup → scale calibration from GPS/odometry → USD terrain for Isaac (procedural fallback if the recon is unusable), per [[pipeline]].
4. **Overnight fine-tune** in Isaac Lab on cloud GPU (the local card is for smoke tests only during the window — see [[setup]]).
